...
export default {
  ...
  methods: {
    init: function () {
      ...
      let vipprice = Cookies.get('vip-price-' + process.env.port)
      if (vipprice === 'yes') {
    ...
    switchState: async function () {
      let isDesigner = Cookies.get('vip-price-' + process.env.port)
      if (isDesigner === 'no') {
        model.isActive = false
    ...
    logout: function () {
      ...
      window.mui.confirm('确定退出登录？', '友情提示', btnArray, function (e) {
        if (e.index === 1) {
          Cookies.set('dpjia-hall-token-' + process.env.port, '', {domain: '.dpjia.com'})
          Cookies.set('dpjia-hall-token-' + process.env.port, '', {domain: '.dpjia.com'})
          Cookies.set('dpjia-exhibite-flag-' + process.env.port, 'no', {domain: '.dpjia.com'})
